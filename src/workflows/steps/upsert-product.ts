...
export const upsertProductStep = createStep(
  "upsert-products",
  async ({ id }: StepInput, { container }) => {
    ...
    const productIndexes = await meilisearchService.getIndexesByType(
      SearchUtils.indexTypes.PRODUCTS
    )
        ...
        if (!product.status || product.status === "published") {
          await Promise.all(
            productIndexes.map((indexKey) =>
              meilisearchService.addDocuments(indexKey, [product])
            )
          )
        } else {
          await Promise.all(
            productIndexes.map((indexKey) =>
              meilisearchService.deleteDocument(indexKey, product.id)
            )
          )
        }
      })
